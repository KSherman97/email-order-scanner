    ...
    for part in message['payload']['parts']:
        if part['filename'] and part['filename'].lower().endswith('.pdf'):
            ...
            if attachment_id:
                ...
                # save the file
                current_file = os.path.join(orders_directory, part['filename'])
...
def check_emails(service, sender_email):
    ...
    email_query = f'from:{sender_email} subject:"INCOMING DELIVERY ORDER #" in:inbox'
                        ...
                        if header['name'] == 'Subject':
                            ...
                            for order in order_numbers:
                                if order_number == order:
                                    print(f"duplicate {order_number}")
                            
                            order_numbers.append(order_number)

                            # mark the email as read
                            service.users().messages().modify(
                                userId='me', id=message['id'], body={'addLabelIds': ['Label_3057771334022643284'],'removeLabelIds': ['INBOX']}
                            ).execute()
